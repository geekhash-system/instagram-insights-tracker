...
- **週次_NERA_2026W02** シート: NERAの週次ダッシュボード（週ごとに自動生成、13週間保持、月曜日始まり）

**KARA子スプレッドシート:**
- **KARA子** シート: KARA子のインサイトデータ（直近30日分）
- **週次_KARA子_2026W02** シート: KARA子の週次ダッシュボード（週ごとに自動生成、13週間保持、月曜日始まり）
...
- **データ保持期間**: Instagram APIは90日分保持していますが、このツールでは30日分のみ取得・保持します
...
- データ保持期間が30日に短縮されるため、古いデータは段階的に削除されます

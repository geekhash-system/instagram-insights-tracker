...
function insertReadmeSheetForAccount(account) {
  try {
    ...
    const readmeContent = [
      ...
      ["   - G列: PR（手動でチェック）⭐重要"],
      ["   - H列: IMP数（ビュー数）"],
      ["   - I列以降: リーチ、いいね、コメント、保存、シェア、エンゲージメント"],
      ["   - P列以降: 日次履歴（「1/13取得」形式で自動記録、永久保存）"],
      [""],
      ["2. アカウントインサイト履歴シート: アカウント全体の指標 ✨NEW"],
      ["   - A列: 日付"],
      ["   - B列: フォロワー数"],
      ...
      ["   - E列: エンゲージしたアカウント数"],
      ["   - F列: 総エンゲージメント数"],
      ["   - G列以降: いいね、コメント、保存、シェア、返信、プロフィールタップ"],
      ["   - 毎日19時に自動更新、全データ永久保存"],
      [""],
      ["3. 週次レポート_mmddシート: 週次ダッシュボード"],
      ["   - 週ごとに自動生成されます（月曜日始まり）"],
      ["   - シート名は作成日の月日（例: 週次レポート_0113）"],
      ...
      ["4. メニュー「📊 インサイト追跡ツール」→「アカウントインサイト取得」（初回のみ）"],
      [""],
      ["■ 日常的な使い方"],
      [`1. ${account.name}シートで最新の投稿インサイトを確認`],
      ["   → C列の曜日で投稿パターンを分析できます"],
      ["2. アカウントインサイト履歴シートでフォロワー推移を確認"],
      ["   → フォロワー増減数（C列）で成長トレンドをチェック"],
      ["3. PR投稿のG列にチェックを入れる（企業タイアップ、案件投稿など）"],
      ["4. 週次ダッシュボードで週ごとの分析を確認"],
      [""],
      ["■ 週次ダッシュボードの見方"],
      ["- 「今週の投稿数」: 月曜〜日曜の投稿数"],
      ["- 「総IMP数」: 全投稿のビュー数合計"],
      ...
      ["【📋 重要な仕様・注意事項】"],
      [""],
      ["✅ データ取得期間: 直近30日分の投稿のみ取得"],
      ["   → 既存データは削除されません（30日より古い投稿も残ります）"],
      [""],
      ["✅ 曜日表示: 各投稿の投稿日から自動計算（C列）"],
      ["   → 月火水木金土日で表示、投稿パターン分析に活用できます"],
      [""],
      ["✅ アカウントインサイト: 毎日自動記録、永久保存"],
      ["   → フォロワー数、エンゲージメント指標を日次で追跡"],
      ["   → フォロワー増減は前日比で自動計算（初日は0）"],
      [""],
      ["✅ 週の定義: 月曜日始まり（月曜〜日曜が1週間）"],
      ["   → 例: 2026/1/6(月) 〜 2026/1/12(日) が第2週"],
      [""],
      ["✅ 週次ダッシュボード: 永久保存"],
      ["   → 週次レポートシートは削除されず、全て残ります"],
      [""],
      ["✅ 履歴データ: P列以降に日次のIMP数推移を記録"],
      ["   → 投稿のIMP数がどう変化したかを追跡できます"],
      ["   → 履歴列も永久保存されます（削除されません）"],
      [""],
